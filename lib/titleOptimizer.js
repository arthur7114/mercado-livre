...
const NON_PROBLEM_TERMS = new Set([
  'preciso', 'precisa', 'consistente', 'claro', 'clara', 'adequado',
  'adequada', 'correto', 'correta', 'validado', 'validada'
]);
...
const PRODUCT_TYPE_PATTERNS = [
  { match: ['sacola', 'viagem'], value: 'Sacola De Viagem' },
  { match: ['sacola'], value: 'Sacola' },
...
function buildDeterministicDescription(input) {
  const attributes = input?.attributes || extractTitleAttributes(input?.title || '');
  const title = compactTitle(input?.title || buildDeterministicTitle({ title: input?.title || '', attributes }));
  const lines = [];
  const descriptorParts = [
    attributes.tipoProduto || title,
    attributes.marca,
    ...(attributes.atributos || [])
  ].filter(Boolean);
  const mainDescription = descriptorParts.length
  ...
  if (attributes.modeloReferencia) {
    lines.push(`Referencia/modelo: ${attributes.modeloReferencia}.`);
  }

  if (attributes.tamanho) {
    lines.push(`Tamanho: ${attributes.tamanho}.`);
  }

  if (attributes.cor) {
    lines.push(`Cor: ${attributes.cor}.`);
  }

  if (attributes.quantidade) {
    lines.push(`Quantidade: ${attributes.quantidade}.`);
  }
...
function validateAiTitle(aiData, context) {
  ...
  const description = normalizeString(aiData?.descricaoOtimizada);
  const flags = [];
  const problems = [];
  const allowedTokens = getAllowedTokens(context);
  const ignoredTokens = new Set([...STOPWORDS, 'un', 'und']);
  const descriptionAllowedTokens = new Set([
    ...ignoredTokens,
    'produto', 'item', 'material', 'referencia', 'modelo', 'tamanho',
    'cor', 'quantidade', 'marca', 'tipo', 'composicao', 'codigo'
  ]);
  ...
  if (description) {
    const descriptionTokens = extractNormalizedTokens(description);
    const prohibitedDescriptionTokens = [...descriptionTokens]
      .filter(token => PROHIBITED_INFERENCES.has(token) && !allowedTokens.has(token));

    if (prohibitedDescriptionTokens.length > 0) {
      flags.push('invented_description_term');
      problems.push(`Descrição contém inferências proibidas: ${prohibitedDescriptionTokens.join(', ')}.`);
    }

    const inventedDescriptionTokens = [...descriptionTokens]
      .filter(token => token.length > 3)
      .filter(token => !allowedTokens.has(token))
      .filter(token => !descriptionAllowedTokens.has(token));

    if (inventedDescriptionTokens.length > 0) {
      flags.push('invented_description_term');
      problems.push(`Descrição contém termos não sustentados pelos dados: ${inventedDescriptionTokens.slice(0, 6).join(', ')}.`);
    }
  }
...
function normalizeAiRegistration(aiData, context, source, validation) {
  ...
  const aiProblems = normalizeStringArray(aiData?.problemasDetectados)
    .filter(problem => !isNonProblemText(problem));
  const guardrailProblems = validation?.problems || [];
  const problems = [
    ...aiProblems,
    ...guardrailProblems
  ];
  const confidence = clampConfidence(aiData?.confidence ?? (source === 'fallback' ? 0.65 : 0.80));
  const humanGate = context.humanGate
    || qualityFlags.length > 0
    || confidence < 0.70
    || aiData?.status === 'blocked'
    || (Boolean(aiData?.humanGate) && problems.length > 0);
  ...
  const finalDescription = normalizeString(aiData?.descricaoOtimizada)
    || context.fallbackDescription
    || buildDeterministicDescription({ title: finalTitle, attributes });
  const removedTerms = [
  ...
  return {
    ...
    descricaoOtimizada: finalDescription,
    status,
    confidence,
    humanGate,
...
function isNonProblemText(value) {
  ...
  return normalized
    .split(' ')
    .filter(Boolean)
    .every(token => NON_PROBLEM_TERMS.has(token) || STOPWORDS.has(token));
}
...
function buildFallbackRegistration(context, source = 'fallback', extraProblems = []) {
  ...
  return normalizeAiRegistration({
    ...
    descricaoOtimizada: context.fallbackDescription,
    status: 'needs_review',
